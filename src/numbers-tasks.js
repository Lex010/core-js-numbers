...
function getHypotenuse(a, b) {
  const absA = a < 0 ? -a : a;
  const absB = b < 0 ? -b : b;
  const maxAbs = absA > absB ? absA : absB;
  const scaledA = a / maxAbs;
  const scaledB = b / maxAbs;
  const squareSum = scaledA ** 2 + scaledB ** 2;
  return maxAbs * Math.sqrt(squareSum);
  // const squareSum = a ** 2 + b ** 2;
